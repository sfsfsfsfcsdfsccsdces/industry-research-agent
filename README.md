...
- 使用 Chroma 向量检索、关键词匹配、来源可信度和跨来源支持度进行混合重排。
- 通过 URL 规范化与内容指纹去重，给每条证据分配稳定 `source_id`。
- 对长上下文分阶段压缩，保留引用锚点，避免直接把全部原文塞给模型。
...
flowchart TD
    ...
    C --> D[Quality Gate<br/>去重·评分·交叉验证]
    D -->|证据不足且未超限| C
    D -->|证据达标| E[Compressor<br/>证据压缩]
    E --> F[Writer<br/>结构化报告]
...
| 本地 RAG | 文档解析 + Chroma + Hash Embedding | 无 Key 时仍可向量检索；生产可替换真实 Embedding |
| 混合重排 | Vector + Keyword + Credibility + Cross-validation | 降低单一召回信号带来的误判 |
...
先规范化 URL、去除追踪参数，再对正文生成 SHA-256 指纹去重。重排分数综合向量相关度、关键词、来源可信度和跨来源支持度；每个来源生成唯一 `source_id`，报告中的引用可反查原文。

### 问题三：长文档直接输入模型导致 Token 过多、关键信息丢失

文档先按标题和段落切分，再检索和重排；报告前执行证据感知压缩，只保留高排名片段与引用锚点。`CONTEXT_CHAR_LIMIT` 控制最终上下文预算。
...
测试覆盖 URL 规范化与去重、文档分块、混合重排、SQLite 持久化，以及从规划到三格式导出的离线端到端工作流。
